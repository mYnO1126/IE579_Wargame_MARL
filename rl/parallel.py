...
def _init_worker(team, obs_dim, act_nvec, global_dim, base_seed, idx_counter, map_size, comp):
    import torch
    ...
    with idx_counter.get_lock():
        idx_counter.value += 1
    _W["env"] = WargameParallelEnv(map_size=map_size, comp=comp, seed=base_seed + 1000 * (wid + 1))
...
class ParallelCollector:
    ...
    def __init__(self, n_workers, team, obs_dim, act_nvec, global_dim, seed=0,
                 map_size=160, comp=None):
        self.n = n_workers
        ctx = mp.get_context("spawn")
        counter = ctx.Value("i", 0)
        self.pool = ctx.Pool(
            n_workers, initializer=_init_worker,
            initargs=(team, obs_dim, act_nvec, global_dim, seed, counter, map_size, comp),
        )
