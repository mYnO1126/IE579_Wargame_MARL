...
| 지형(8방향) | 8방향 × {이동가능거리, 평균 험준함, 고도변화} | 8×3 = 24 |
...
- 지형은 11×11 패치(363, CNN 필요) 대신 **8방향 압축 특징**(각 방향 이동가능거리/평균 험준함/고도변화)으로
  표현 → **CNN 불필요, MLP로 충분**(CNN vs MLP 결정 해소). 고지/험지/장애물 정보가 방향별로 압축됨.
...
- goal 접근: 거리 감소량에 소량 양(+) (예 `+0.01 × Δ거리_px`), 도달 시 보너스.
  **goal 보유 유닛(RED)만 적용** — BLUE(방어)는 goal 없음 → 전투 보상만으로 방어 행동 학습.
...
- ~~goal point는 누가?~~ → **결정됨**: 기존 시나리오의 per-troop `fixed_dest` 사용. RED(공격)만
  goal 보유, BLUE(방어)는 없음(웬만하면 정지). 학습 크롭에선 RED 에 BLUE 진영 쪽 goal 자동 부여.
- hold-fire(은폐 전술) 도입 시점.
- ~~지형패치 CNN vs MLP~~ → **결정됨: MLP**(지형을 8방향 압축 특징 24차원으로, §4).
- 보상 계수·정규화 스케일.
